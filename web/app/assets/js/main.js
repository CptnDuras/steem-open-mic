...
import PlayPage                 from '../vue/PlayPage.vue'
import SearchResultsPage        from '../vue/SearchResultsPage.vue'
import SearchResultsListPage    from '../vue/SearchResultsListPage.vue'
import ThumbnailsPage           from '../vue/ThumbnailsPage.vue'
import HeaderBar                from '../vue/HeaderBar.vue'
import PlayerYoutube            from '../vue/PlayerYoutube.vue'
import PlayerDtube              from '../vue/PlayerDtube.vue'
import PlayerDlive              from '../vue/PlayerDlive.vue'
import RepliesPanel             from '../vue/RepliesPanel.vue'
...
const globals = new Vue({
    data: function() {
        return {
            ...
            Icon: Icon,
            default_vote_percent: 50,  // todo - save to, and load this from local storage
            search_terms: '',
...
//                  let metadata = JSON.stringify(result.user_metadata, null, 2);
//                  console.log(result);
...
//                    console.log(metadata);
        ...
        getVotesInfo: function(authur, permlink, success_callback, error_callback) {
            ...
            steem.api.getContent(authur, permlink, function(err, result) {
                if (!err) {
                    let total_payout = parseFloat(result.pending_payout_value) + parseFloat(result.total_payout_value);
                    ...
                    let vote_contributions = votes.map(function(x) {
                        let c =  total_payout * (parseInt(x.rshares) / total_rshares);
                        if (c > 0) {
